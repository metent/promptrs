...
function readFileSync(path, encoding, dirfd) {
	dirfd ??= descriptor();
	const fd = dirfd.openAt({}, path, {}, { read: true });
	const [bytes, _] = fd.read(10000, 0);
	return new TextDecoder().decode(bytes);
}

function readdirSync(path, dirfd) {
	dirfd ??= descriptor().openAt({}, path, { directory: true }, {
		read: true,
		mutateDirectory: true,
	});
	let dirent;
	let iter = dirfd.readDirectory();
	while (
		(dirent = iter.readDirectoryEntry()) && dirent?.type !== "regular-file" ||
		dirent?.name.startsWith("knowledge")
	);
	return [dirent?.name, dirfd];
}

function mkdirSync(path) {
	descriptor().createDirectoryAt(path);
}

function rmSync(path, dirfd) {
	dirfd ??= descriptor();
...
tools.updateTask = ({
	...
	try {
		...
		return `<tool_response>${JSON.stringify({ error: err })}</tool_response>\n`;
	}
};
Object.assign(tools.updateTask, {
...
export const getStatus = () => {
	let text = readFileSync("knowledge.org", "utf8");
	return `Current time: ${
		new Date().toString()
...
function findTaskEnd(lines, startLine) {
